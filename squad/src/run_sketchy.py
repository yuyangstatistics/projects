...
def main(args):
    ...
    scheduler = sched.LambdaLR(optimizer, lambda epoch: 0.85 ** epoch)  # Constant LR
        ...
        with torch.enable_grad(), \
            ...
            for cw_idxs, cc_idxs, qw_idxs, qc_idxs, y1, y2, ids in train_loader:
                ...
                progress_bar.set_postfix(epoch=epoch,
                                         BCE=loss_val)
                tbx.add_scalar('train/BCE', loss_val, step)
...
def evaluate(model, data_loader, device, eval_file, max_len, use_squad_v2):
    ...
    with torch.no_grad(), \
        ...
        for cw_idxs, cc_idxs, qw_idxs, qc_idxs, y1, y2, ids in data_loader:
            ...
            starts = (logits > 0.5).int().to(device)
            ends = (logits > 0.5).int().to(device) * 2
    ...
    results_list = [('BCE', bce_meter.avg)]
    if use_squad_v2:
