...
def visualize(tbx, pred_topics, step, split, vis_num_topics):
    ...
    Args:
        ...
        vis_num_topics (int): Number of topics to select to visualize.
    """
    if vis_num_topics <= 0:
        return
    if vis_num_topics > len(pred_topics):
        vis_num_topics = len(pred_topics)

    np.random.seed(99)
    visual_ids = np.random.choice(list(pred_topics), size=vis_num_topics, replace=False)

    for i, visual_id in enumerate(visual_ids):
        pred_topic_words = pred_topics[visual_id]
        tbl_fmt = (f'- **Topic {visual_id}:** {pred_topic_words}\n')
        tbx.add_text(tag=f'{split}/{i+1}_of_{vis_num_topics}',
                     text_string=tbl_fmt,
                     global_step=step)
...
def tav(p_start, p_end, v, lambda1, lambda2, threshold, max_len=15):
    ...
    # Predict no-answer whenever score_diff > threshold
    score_has, _ = torch.max(max_in_col, dim=-1)
    score_na = lambda1 * p_no_answer + lambda2 * v * v
    score_diff = score_has - score_na

    start_idxs[score_diff < threshold] = 0
    end_idxs[score_diff < threshold] = 0

    return start_idxs, end_idxs


# ==========================  ETM Utility Functions ====================================== # 
...
def get_outer_triu_values(x):
    """ Get the values of the triangular part (diagonal included) 
        of the outer product matrix.

    x: a one-dimensional tensor

    return: a one-dimensional tensor of length len(x) * (len(x) + 1) / 2
    """
    return torch.outer(x, x)[torch.triu_indices(len(x), len(x)).unbind()]

def get_topic_coherence(frequency, D):
    TC = []
    diag_indices = [(21 - i) * i // 2 for i in range(10)] # indices should be integers, indices that correspond to the diagonal elements in the outer product matrix
    for k in range(frequency.shape[0]):
        TC_k = 0
        for i in range(10):
            D_wi = frequency[k, diag_indices[i]]
            for j in range(i+1, 10):
                D_wj = frequency[k, diag_indices[j]]
                D_wi_wj = frequency[k, diag_indices[i] + j - i]
                if D_wi_wj == 0:
                    f_wi_wj = -1
                else:
                    with np.errstate(all='raise'):
                        try:
                            ...
                            print(f"Caught an error in TC computation: {e}")
                            print(f"D_wi: ", {D_wi}, " D_wj: ", {D_wj}, " D_wi_wj: ", {D_wi_wj}, " D: ", {D})
                            f_wi_wj = -1
                TC_k += f_wi_wj
        TC_k = TC_k / 45
        TC.append(TC_k)
...
def get_topic_diversity(beta, topk):
    top_k_indicies = beta.argsort(dim = -1, descending=True)[:, :topk]  # (K x k)
    n_unique = len(torch.unique(top_k_indicies))
    TD = n_unique / (top_k_indicies.shape[0] * topk)
    return TD

def get_topics(beta, tokenizer, vis_num_words):
    top_word_indicies = beta.argsort(dim = -1, descending=True)[:, :vis_num_words].data.cpu().numpy()
    topics_words = {k: [tokenizer.decode(int(w_id)) for w_id in top_word_indicies[k, :]] for k in range(beta.size(0))}
    return OrderedDict(topics_words)

class dotdict(dict):
    """dot.notation access to dictionary attributes. 
    Used for creating args in debug."""
