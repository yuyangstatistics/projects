...
class Encoder(nn.Module):
    ...
    def forward(self, input, seq_lens):
        ...
        # transform seq_lens tensor to list
        ...
        encoder_outputs, _ = pad_packed_sequence(output, batch_first=True, \
            total_length=total_length)  # h dim = B x t_k x n
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        if config.add_rbm:
            self.out3 = nn.Linear(config.latent_dim, config.vocab_size, bias=False)
            init_linear_wt(self.out3)

    def forward(self, y_t_1, s_t_1, encoder_outputs, encoder_feature, enc_padding_mask,
                c_t_1, extra_zeros, enc_batch_extend_vocab, coverage, step, latent_batch=None):
        ...
        if config.add_rbm:
            output = self.out2(output) + self.out3(latent_batch) # B x vocab_size
        else: 
